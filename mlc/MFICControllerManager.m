...
@interface MFICControllerManager ()
...
@implementation MFICControllerManager{
...
+ (id)sharedManager
{
    ...
    static MFICControllerManager *shared = nil;
    if(shared == nil)
    {
        dispatch_once(&onceToken, ^{
            shared = [[MFICControllerManager alloc] init];
...
/**
 ...
 all these are defined in MFICDefines.h
...
- (NSDictionary *)controllerPreferences {
    ...
    if (self.gamePlayDictionary == nil){
        NSString *preferenceFile = @"/var/mobile/Library/Preferences/com.nito.mlc.plist";
        NSString *localFile = [[self documentsFolder] stringByAppendingPathComponent:@"com.nito.mlc.plist"];
        NSFileManager *man = [NSFileManager defaultManager];
        if (![man fileExistsAtPath:localFile]){
            [man copyItemAtPath:preferenceFile toPath:localFile error:nil];
        }
        NSLog(@"MLC: localFile: %@", localFile);
...
- (void)updateGamplayValue:(id)value forKey:(NSString *)theKey {
    ...
    if (theKey != nil && value != nil){
        ...
        NSString *preferenceFile = [[self documentsFolder] stringByAppendingPathComponent:@"com.nito.mlc.plist"];
...
- (void)startButton {
    ...
    CGPoint start = PAT(kMLActionStartButton);
...
- (void)setupTapRecognizerIfNeeded {
    ...
    if (view != nil && _tapSetup == FALSE){
        ...
        _tapSetup = TRUE;
        NSLog(@"#### show alert??");
        [RKDropdownAlert title:@"MLC 1.0 Activated" message:@"Tap here now OR double tap anywhere on the screen with two fingers to bring up the control customization window." backgroundColor:[UIColor redColor] textColor:[UIColor whiteColor] time:3 delegate:self];
    }
}
...
- (MLActionType)actionTypeFromConstant:(NSString *)constant {
    ...
    MLActionType type = kMLActionTypeUndefined;
    if ([constant isEqualToString:MLCallForBackup]){
        type = kMLCallForBackup;
    } else if ([constant isEqualToString:MLSkillOne]){
        type = kMLSkillOne;
    } else if ([constant isEqualToString:MLSkillTwo]){
        type = kMLSkillTwo;
    } else if ([constant isEqualToString:MLSkillThree]){
        type = kMLSkillThree;
    } else if ([constant isEqualToString:MLWeaponAttack]){
        type = kMLWeaponAttack;
    } else if ([constant isEqualToString:MLRetreat]){
        type = kMLRetreat;
    }else if ([constant isEqualToString:MLOrderAttack]){
        type = kMLOrderAttack;
    }else if ([constant isEqualToString:MLTopRightAction]){
        type = kMLTopRightAction;
    }else if ([constant isEqualToString:MLRegen]){
        type = kMLRegen;
    }else if ([constant isEqualToString:MLRecall]){
        type = kMLRecall;
    }else if ([constant isEqualToString:MLMagGlass]){
        type = kMLMagGlass;
    }else if ([constant isEqualToString:MLStore]){
        type = kMLStore;
    }else if ([constant isEqualToString:MLChat]){
        type = kMLChat;
    }else if ([constant isEqualToString:MLOKButton]){
        type = kMLOKButton;
    }else if ([constant isEqualToString:MLContinue]){
        type = kMLContinue;
    }else if ([constant isEqualToString:MLSuggestedItemOne]){
        type = kMLSuggestedItemOne;
    }else if ([constant isEqualToString:MLSuggestedItemTwo]){
        type = kMLSuggestedItemTwo;
    }else if ([constant isEqualToString:MLSkillOnePlus]){
        type = kMLSkillOnePlus;
    }else if ([constant isEqualToString:MLSkillTwoPlus]){
        type = kMLSkillTwoPlus;
    }
...
- (CGPoint)pointForActionType:(MLActionType)type {
    ...
    switch (type) {
        case kMLWeaponAttack:
            outpoint = [self convertPointForScreen:CGPointMake(618,326)];
            break;
            
        case kMLSkillOne:
            outpoint = [self convertPointForScreen:CGPointMake(512,327)];
            break;
            
        case kMLSkillTwo:
            outpoint = [self convertPointForScreen:CGPointMake(545,257)];
            break;
            
        case kMLSkillThree:
            outpoint = [self convertPointForScreen:CGPointMake(616,220)];
            break;
            
        case kMLCallForBackup:
            outpoint = [self convertPointForScreen:CGPointMake(651, 102)];
            break;
    
        case kMLRetreat:
            outpoint = [self convertPointForScreen:CGPointMake(651, 73)];
            break;
            
        case kMLOrderAttack:
            outpoint = [self convertPointForScreen:CGPointMake(651,42)];
            break;
            
        case kMLTopRightAction:
            outpoint = [self convertPointForScreen:CGPointMake(645,20)];
            break;
            
        case kMLRegen:
            outpoint = [self convertPointForScreen:CGPointMake(394,333)];
            break;
            
        case kMLRecall:
            outpoint = [self convertPointForScreen:CGPointMake(342,333)];
            break;
         
        case kMLMagGlass:
            outpoint = [self convertPointForScreen:CGPointMake(18,140)];
            break;
            
        case kMLStore:
            outpoint = [self convertPointForScreen:CGPointMake(644,13)];
            break;
            
        case kMLChat:
            outpoint = [self convertPointForScreen:CGPointMake(651, 132)];
            break;
            
        case kMLOKButton:
            outpoint = [self convertPointForScreen:CGPointMake(593,352)];
            break;
            
        case kMLContinue:
            outpoint = [self convertPointForScreen:CGPointMake(332,351)];
            break;
            
        case kMLSuggestedItemOne:
            outpoint = [self convertPointForScreen:CGPointMake(611,52)];
            break;
            
        case kMLSuggestedItemTwo:
            outpoint = [self convertPointForScreen:CGPointMake(611,95)];
            break;
            
        case kMLSkillOnePlus:
            outpoint = [self convertPointForScreen:CGPointMake(475,294)];
            break;
            
        case kMLSkillTwoPlus:
            outpoint = [self convertPointForScreen:CGPointMake(515,225)];
            break;
            
    
        case kMLActionStartButton:
            outpoint = [self convertPointForScreen:CGPointMake(184,258)];
            break;
            
        case kMLActionMenuButton:
